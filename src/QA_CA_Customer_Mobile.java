...
public class QA_CA_Customer_Mobile  {
	...
	public QA_CA_Customer_Mobile(String a, String b, String c, String d, String e, boolean f, boolean g, boolean h, boolean i){
		usr = a;
		pw = b;
		ccVeri = c;
		securityAns=d;
	...
	public void screenShot(String error){
		...
		try {
			FileUtils.copyFile(scrFile, new File("C:\\BOSS_Screenshots_"+ bossVersion+"\\CA_Mobile_"+error+".png"));
			pics[picCount++]="C:\\BOSS_Screenshots_"+ bossVersion+"\\CA_Mobile_"+error+".png";
		} catch (IOException e) {
	...
	public boolean createCust(){
		...
		do{
			...
			driver.findElement(By.xpath("//form[@id='user_session_new']/div[6]/a/span/span")).click();
			...
			driver.findElement(By.id("business_entity_user_attributes_first_name")).sendKeys("Tony");
	...
	public boolean recharge(){
		...
		//driver.findElement(By.xpath("//div[@id='page']/div[3]/ul/li[2]/div/div/a/span[2]")).click();
	...
	public boolean raf(){
		...
		driver.findElement(By.cssSelector("a.menu.ui-link")).click();
		driver.findElement(By.xpath("//a[contains(text(),'Friends Forever')]")).click();
		//driver.findElement(By.xpath("//a[contains(text(),'Refer A Friend')]")).click();
		try {
			assertEquals("Boss Revolution Customers - Friends Forever", driver.getTitle());
		} catch (Error e) {
			return false;
		}
		driver.findElement(By.cssSelector("#invite_btn > span.ui-btn-inner")).click();
		try {
			assertEquals("Start earning FREE Calling Rewards for referring friends.", driver.findElement(By.xpath("//form[@id='form_raf']/div[2]")).getText());
		} catch (Error e) {
			return false;
		}
		return true;
	...
	public boolean mobileApp(){
		...
		try {
			assertTrue(isElementPresent(By.cssSelector("img[alt=\"Mobile_app_mobile_en-ca\"]")));
		} catch (Error e) {
			return false;
		}
		try {
	...
	public boolean help(){
		...
		try {
			assertEquals("By Phone", driver.findElement(By.xpath("//div[@id='page']/div[3]/div[3]/div")).getText());
		} catch (Error e) {
			return false;
		}
		try {
			assertEquals("Boss Revolution Retailers", driver.findElement(By.xpath("//div[@id='page']/div[3]/div[5]/div")).getText());
		} catch (Error e) {
			return false;
		}
		try {
			assertTrue(isElementPresent(By.xpath("//div[@id='page']/div[3]/div[5]/div[2]/p")));
		} catch (Error e) {
			return false;
		}
		...
		try {
			assertEquals("Boss Revolution Customers - Frequently Asked Questions", driver.getTitle());
		} catch (Error e) {
			return false;
		}
		try {
			assertTrue(isElementPresent(By.linkText("General")));
		} catch (Error e) {
			return false;
		}
		try {
			assertTrue(isElementPresent(By.linkText("International Calling")));
		} catch (Error e) {
			return false;
		}
		try {
			assertTrue(isElementPresent(By.linkText("Mobile Top-Up")));
		} catch (Error e) {
			return false;
		}
		try {
		...
		try {
			assertTrue(isElementPresent(By.linkText("Friends Forever")));
		} catch (Error e) {
			return false;
		}
		...
		try {
			assertTrue(isElementPresent(By.id("PC4")));
		} catch (Error e) {
			return false;
		}
		try {
	...
	public boolean imtu(){
		...
		q.selectByVisibleText("Ghana");
		driver.findElement(By.id("receiver")).clear();
		driver.findElement(By.id("receiver")).sendKeys("0265658898");
	...
	public boolean test() throws Exception {
		...
		if(creation)
			if(createCust()){
				System.out.println("[Sucess]Created new Customer Successfully");
	...
	public void print(){
		...
		System.out.println("");
		System.out.println("**CA Customer Mobile**");
		if(update||payment||links)
			//login//////////////////////
			if(result[1]==1){
		...
		if(links){
			//Navigation//////////////////////
			if(result[2]==1 && result[8]==1 && result[13]==1 && result[14]==1 && result[15]==1 && result[17]==1){
