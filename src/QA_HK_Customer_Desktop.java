...
public class QA_HK_Customer_Desktop  {
	
	public static void main (String[] args){
		System.out.println("qa customer desktop class");
	}
	...
	public QA_HK_Customer_Desktop(String a, String b, String c, String d, String e, boolean f, boolean g, boolean h, boolean i){
		usr = a;
		pw = b;
		ccVeri = c;
		securityAnswer = d;
	...
	public void screenShot(String error){
		...
		try {
			FileUtils.copyFile(scrFile, new File("C:\\BOSS_Screenshots_"+ bossVersion+"\\HK_Customer_"+error+".png"));
			pics[picCount++]="C:\\BOSS_Screenshots_"+ bossVersion+"\\HK_Customer_"+error+".png";
		} catch (IOException e) {
	...
	public boolean recharge(){
		...
		if(isElementPresent(By.cssSelector("p")))
			if("Your account is not setup for credit card recharges. Please contact Customer Services.".equals(driver.findElement(By.cssSelector("p")).getText()))
				return false;
		try {
	...
	public boolean vcc(){
		...
		for (int second = 0;; second++) {
			...
			try { if (isElementPresent(By.cssSelector("img[alt=\"En-hk_vcard\"]"))) break; } catch (Exception e) {}
			try {
				Thread.sleep(1000);
		...
		try {
			assertTrue(isElementPresent(By.cssSelector("img[alt=\"En-hk_vcard\"]")));
		} catch (Error e) {
			return false;
		}
		return true;
	...
	public boolean rates(){
		...
		try {
			assertEquals("One Step Dialling", driver.findElement(By.xpath("//div[@id='content']/table/tbody/tr/td[2]/table/tbody/tr/td[2]/div[3]")).getText());
		} catch (Error e) {
			return false;
		}
		return true;
	...
	public boolean mobileApp(){
		...
		try {
			assertTrue(isElementPresent(By.cssSelector("img[alt=\"Mobile_app_desktop_en-hk\"]")));
		} catch (Error e) {
			return false;
		}
		return true;
	...
	public void print(){
		...
		System.out.println("");
		System.out.println("**HK Customer Desktop**");
		if(update||payment||links)
			//login//////////////////////
			if(result[1]==1){
		...
		if(links){
			...
			////////////////
			if(result[2]==1 && result[7]==1 && result[9]==1 && result[12]==1 && result[13]==1 && result[14]==1 && result[19]==1 && result[18]==1 && result[20]==1 && result[21]==1 && result[22]==1 && result[25]==1 && result[26]==1 && result[27]==1){
