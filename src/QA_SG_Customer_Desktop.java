...
public class QA_SG_Customer_Desktop  {
	...
	public QA_SG_Customer_Desktop(String a, String b, String c, String d, String e, boolean f, boolean g, boolean h, boolean i){
		usr = a;
		pw = b;
		ccVeri = c;
		securityAnswer = d;
	...
	public void setUp() throws Exception {
		...
		if(qa==0){
			baseUrl = "http://sg.bossqa.cstage01.n2p.com/";
		}else{
			baseUrl = "http://sg.bossqa"+qa+".cstage01.n2p.com/";
		}
	...
	public void screenShot(String error){
		...
		try {
			FileUtils.copyFile(scrFile, new File("C:\\BOSS_Screenshots_"+ bossVersion+"\\SG_Customer_"+error+".png"));
			pics[picCount++]="C:\\BOSS_Screenshots_"+ bossVersion+"\\SG_Customer_"+error+".png";
		} catch (IOException e) {
	...
	public boolean createCust(){
		...
		do{
			...
			driver.findElement(By.id("business_entity_user_attributes_first_name")).sendKeys("g");
			driver.findElement(By.id("business_entity_user_attributes_last_name")).clear();
			driver.findElement(By.id("business_entity_user_attributes_last_name")).sendKeys("g");
			...
			driver.findElement(By.id("business_entity_physical_addresses_attributes_0_city")).sendKeys("Singapore");
			driver.findElement(By.id("business_entity_physical_addresses_attributes_0_zip_code")).clear();
			driver.findElement(By.id("business_entity_physical_addresses_attributes_0_zip_code")).sendKeys("123456");
	...
	public boolean recharge(){
		...
		try {
			assertEquals("Your account is not setup for credit card recharges. Please contact Customer Services.", driver.findElement(By.cssSelector("p")).getText());
			//result[3]=2;
			return false;
		} catch (Error e) {
	...
	public String randNum(String w){
		String q;
		do
			q="65"+String.valueOf((int)(Math.floor((Math.random() * 10000000)+89999999)));
	...
	public boolean vcc(){
		...
		for (int second = 0;; second++) {
			...
			try { if (isElementPresent(By.cssSelector("img[alt=\"En-sg_vcard\"]"))) break; } catch (Exception e) {}
			try {
				Thread.sleep(1000);
		...
		try {
			assertTrue(isElementPresent(By.cssSelector("img[alt=\"En-sg_vcard\"]")));
		} catch (Error e) {
			return false;
		}
		return true;
	...
	public boolean imtu(){
		...
		try {
			assertEquals("Your credit card on file can not be used for topups. Please update you credit card in Personal Details or contact Customer Services.", driver.findElement(By.cssSelector("p")).getText());
			//result[15]=2;
			return false;
		} catch (Error e) {
		...
		driver.findElement(By.cssSelector("option[value=\"NG\"]")).click();
	...
	public boolean rates(){
	    ...
	    try {
	      assertEquals("Minutes Calculator", driver.findElement(By.cssSelector("div.form_title")).getText());
	    } catch (Error e) {
			return false;
	    }
	    try {
	      assertEquals("Our Top 5", driver.findElement(By.xpath("//div[@id='content']/table/tbody/tr/td[2]/table/tbody/tr/td[2]/div")).getText());
	    } catch (Error e) {
			return false;
	    }
	    try {
	      assertEquals("Access Number", driver.findElement(By.xpath("//div[@id='content']/table/tbody/tr/td[2]/table/tbody/tr/td[2]/div[3]")).getText());
	    } catch (Error e) {
			return false;
	    }
		return true;
	...
	public boolean mobileApp(){
		...
		try {
			assertTrue(isElementPresent(By.cssSelector("img[alt=\"Mobile_app_desktop_en-sg\"]")));
		} catch (Error e) {
			return false;
		}
		return true;
	...
	public void print(){
		...
		System.out.println("");
		System.out.println("**SG Customer Desktop**");
		if(update||payment||links)
			//login//////////////////////
			if(result[1]==1){
		...
		if(links){
			...
			////////////////
			if(result[2]==1 && result[7]==1 && result[9]==1 && result[12]==1 && result[13]==1 && result[14]==1 && result[18]==1 && result[19]==1 && result[20]==1 && result[21]==1 && result[22]==1 && result[25]==1 && result[26]==1 && result[27]==1){
