...
public class QA_AU_Retailer  {
	...
	public QA_AU_Retailer(String a, String b, String c, boolean d, boolean e, boolean f, boolean g, int h){
		usr = a;
		pw = b;
		bossVersion = c;
		update = d;
	...
	public void screenShot(String error){
		...
		case 0:
			try {
				FileUtils.copyFile(scrFile, new File("C:\\BOSS_Screenshots_"+ bossVersion+"\\AU_Retailer_"+error+".png"));
				pics[picCount++]="C:\\BOSS_Screenshots_"+ bossVersion+"\\AU_Retailer_"+error+".png";
			} catch (IOException e) {
		...
		case 1:
			try {
				FileUtils.copyFile(scrFile, new File("C:\\BOSS_Screenshots_"+ bossVersion+"\\AU_Cash Retailer_"+error+".png"));
				pics[picCount++]="C:\\BOSS_Screenshots_"+ bossVersion+"\\AU_Cash Retailer_"+error+".png";
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
		...
		case 2:
			try {
				FileUtils.copyFile(scrFile, new File("C:\\BOSS_Screenshots_"+ bossVersion+"\\AU_Credit Retailer_"+error+".png"));
				pics[picCount++]="C:\\BOSS_Screenshots_"+ bossVersion+"\\AU_Credit Retailer_"+error+".png";
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
	...
	public boolean homePage(){
		...
		try {
			assertEquals("Boss Revolution Retailers - Home", driver.getTitle());
		} catch (Error e) {
			return false;
		}
		try {
	...
	public void createCust2(){
		driver.findElement(By.linkText("Manage My Customers")).click();
	...
	public boolean recharge(){
		...
		driver.findElement(By.name("commit")).click();
		if(isElementPresent(By.cssSelector("div.toast-item.toast-type-error > p"))) return false;
		if(driver.findElement(By.cssSelector("div.toast-item.toast-type-success > p")).getText().contains("successful."))return true;
		return false;
	}
	...
	public boolean imtu(){
		...
		driver.findElement(By.name("imtu_request[receiver_phone]")).sendKeys("2348077879985");
	...
	public boolean agents(){
		...
		driver.findElement(By.linkText("Manage My Agents")).click();
		try {
			assertEquals("Manage My Agents", driver.findElement(By.cssSelector("h1")).getText());
		} catch (Error e) {
			return false;
		}
	...
	public boolean ratesAccess(){
		...
		new Select(driver.findElement(By.id("rate_check_country"))).selectByVisibleText("Montenegro");
		try {
			assertEquals("Montenegro", driver.findElement(By.cssSelector("tr.even > td.l")).getText());
		} catch (Error e) {
			return false;
		}
		try {
			assertEquals("AU Sydney", driver.findElement(By.cssSelector("div.iform > div > table.result > tbody > tr.odd > td")).getText());
		} catch (Error e) {
			return false;
		}
		return true;
	...
	public boolean refund(){
		...
		if(isElementPresent(By.xpath("//div[@id='content']/table/tbody/tr/td[2]")))
			if("Please call Customer Services at 1300 859 438.".equals(driver.findElement(By.xpath("//div[@id='content']/table/tbody/tr/td[2]")).getText())){
				result[30]=2;
				return true;
			}
		driver.findElement(By.id("amount")).clear();
	...
	public void print(){
		...
		System.out.println("");
		System.out.println("**AU Retailer**");
		//login//////////////////////
		if(result[1]==1){
		...
		if(creation){
			...
			////////////////////////
			if(result[3]==1)
			if(result[4]==1 && result[5]==1 && result[6]==1 && result[7]==1 && result[9]==1 && result[10]==1 && result[31]==1){
