...
public class QA_AU_Customer_Desktop  {
	...
	public QA_AU_Customer_Desktop(String a, String b, String c, String d, String e, boolean f, boolean g, boolean h, boolean i){
		usr = a;
		pw = b;
		ccVeri = c;
		securityAnswer = d;
	...
	public void screenShot(String error){
		...
		try {
			FileUtils.copyFile(scrFile, new File("C:\\BOSS_Screenshots_"+ bossVersion+"\\AU_Customer_"+error+".png"));
			pics[picCount++]="C:\\BOSS_Screenshots_"+ bossVersion+"\\AU_Customer_"+error+".png";
		} catch (IOException e) {
	...
	public boolean login(){
		...
		try {
			...
			try {
				assertEquals("Customer Information", driver.findElement(By.id("subheader_C")).getText());
				driver.findElement(By.id("first_name")).clear();
				driver.findElement(By.id("first_name")).sendKeys("qwe");
				driver.findElement(By.id("last_name")).clear();
				driver.findElement(By.id("last_name")).sendKeys("qwe");
	...
	public boolean vcc(){
		...
		for (int second = 0;; second++) {
			...
			try { if (isElementPresent(By.cssSelector("img[alt=\"En-au_vcard\"]"))) break; } catch (Exception e) {}
			try {
				Thread.sleep(1000);
		...
		try {
			assertTrue(isElementPresent(By.cssSelector("img[alt=\"En-au_vcard\"]")));
		} catch (Error e) {
			return false;
		}
		return true;
	...
	public boolean imtu(){
		...
		driver.findElement(By.id("receiver")).sendKeys("2348077879985");
	...
	public boolean rates(){
		...
		new Select(driver.findElement(By.id("rate_check_country"))).selectByVisibleText("Mozambique");
		try {
			assertEquals("Mozambique", driver.findElement(By.cssSelector("td.l")).getText());
		} catch (Error e) {
			return false;
		}
		new Select(driver.findElement(By.id("rate_check_country"))).selectByVisibleText("Slovakia");
		try {
			assertEquals("Slovakia", driver.findElement(By.cssSelector("td.l")).getText());
		} catch (Error e) {
			return false;
		}
		try {
	...
	public boolean mobileApp(){
		...
		try {
			assertTrue(isElementPresent(By.cssSelector("img[alt=\"Mobile_app_desktop_en-au\"]")));
		} catch (Error e) {
			return false;
		}
		return true;
	...
	public void print(){
		...
		System.out.println("");
		System.out.println("**AU Customer Desktop**");
		if(update||payment||links){
			//login//////////////////////
			if(result[1]==1){
