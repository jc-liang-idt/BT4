...
public class QA_DE_Customer_Desktop  {
	...
	public QA_DE_Customer_Desktop(String a, String b, String c, String d, String e, boolean update, boolean payment, boolean links, boolean q){
		usr = a;
		pw = b;
		ccVeri = c;
		secAns = d;
	...
	public void screenShot(String error){
		...
		try {
			FileUtils.copyFile(scrFile, new File("C:\\BOSS_Screenshots_"+ bossVersion+"\\DE_Customer_"+error+".png"));
			pics[picCount++]="C:\\BOSS_Screenshots_"+ bossVersion+"\\DE_Customer_"+error+".png";
		} catch (IOException e) {
	...
	public boolean homePage(){
		...
		try {
			assertTrue(isElementPresent(By.cssSelector("div.acct.tab_image")));
		} catch (Error e) {
			return false;
		}
		try {
			assertTrue(isElementPresent(By.xpath("//div[@id='C_T']/div/a[4]/div[2]")));
		} catch (Error e) {
			return false;
		}
		return true;
	...
	public boolean vcc(){
		...
		try {
			assertTrue(isElementPresent(By.cssSelector("img[alt=\"En-de_vcard\"]")));
		} catch (Error e) {
			return false;
		}
		return true;
	...
	public boolean imtu(){
		...
		q.selectByValue(q.getOptions().get(1).getAttribute("value"));
		//new Select(driver.findElement(By.id("product"))).selectByVisibleText("€9.00");
		driver.findElement(By.id("receiver")).clear();
		driver.findElement(By.id("receiver")).sendKeys("4924564578");
	...
	public boolean rates(){
		...
		try {
			assertEquals("Boss Revolution Customers - Rates", driver.getTitle());
		} catch (Error e) {
			return false;
		}
		try {
		...
		new Select(driver.findElement(By.id("rate_check_country"))).selectByVisibleText("Bahamas");
		try {
			assertEquals("Bahamas", driver.findElement(By.cssSelector("td.l")).getText());
		} catch (Error e) {
			return false;
		}
		try {
			assertEquals("India", driver.findElement(By.xpath("//div[@id='content']/table/tbody/tr/td[2]/table/tbody/tr/td[2]/div[2]/table/tbody/tr[2]/td")).getText());
		} catch (Error e) {
			return false;
		}
	/*	try {
			assertTrue(isElementPresent(By.xpath("//div[@id='content']/table/tbody/tr/td[2]/table/tbody/tr/td[2]/div[4]/div[2]/table/tbody/tr/td")));
		} catch (Error e) {
			return false;
		}*/
		return true;
	...
	public boolean mobileApp(){
		...
		try {
			assertTrue(isElementPresent(By.cssSelector("img[alt=\"Mobile_app_desktop_en-de\"]")));
		} catch (Error e) {
			return false;
		}
		return true;
	...
	public boolean raf2(){
		...
		try {
			assertTrue(isElementPresent(By.cssSelector("img[alt=\"Ff_desktop_en-de\"]")));
		} catch (Error e) {
			return false;
		}
		try {
	...
	public boolean reseller(){
		...
		driver.findElement(By.linkText("Become A Reseller")).click();
		try {
	...
	public void print(){
		...
		System.out.println("");
		System.out.println("**DE Customer Desktop**");
		if(update||payment||links)
			//login//////////////////////
			if(result[1]==1){
