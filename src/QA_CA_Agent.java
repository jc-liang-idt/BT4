...
public class QA_CA_Agent  {
	...
	public QA_CA_Agent(String a, String b, String c, String d, boolean e, boolean f, boolean g){
		usr = a;
		pw = b;
		secAns= c;
		bossVersion = d;
	...
	public void screenShot(String error){
		...
		try {
			FileUtils.copyFile(scrFile, new File("C:\\BOSS_Screenshots_"+ bossVersion+"\\CA_Agent_"+error+".png"));
			pics[picCount++]="C:\\BOSS_Screenshots_"+ bossVersion+"\\CA_Agent_"+error+".png";
		} catch (IOException e) {
	...
	public boolean homePage(){
		...
		try {
			assertEquals("Store Name", driver.findElement(By.cssSelector("td.cap")).getText());
		} catch (Error e) {
			return false;
		}
		try {
	...
	public boolean imtu(){
		...
		driver.findElement(By.name("imtu_request[receiver_phone]")).sendKeys("233289115922"); //sendKeys("50212344567");
	...
	public boolean ratesAccess(){
		...
		new Select(driver.findElement(By.id("rate_check_country"))).selectByVisibleText("Mauritius");
		try {
			assertEquals("Mauritius", driver.findElement(By.cssSelector("tr.even > td.l")).getText());
		} catch (Error e) {
			return false;
		}
		new Select(driver.findElement(By.id("rate_check_country"))).selectByVisibleText("Tunisia");
		try {
			assertEquals("Tunisia", driver.findElement(By.cssSelector("tr.even > td.l")).getText());
		} catch (Error e) {
			return false;
		}
		return true;
