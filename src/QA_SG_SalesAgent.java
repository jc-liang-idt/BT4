...
public class QA_SG_SalesAgent {
	...
	public QA_SG_SalesAgent(String a, String b, String c, boolean d, boolean e, boolean f, boolean g){
		usr = a;
		pw = b;
		bossVersion = c;
		update = d;
	...
	public void screenShot(String error){
		...
		try {
			FileUtils.copyFile(scrFile, new File("C:\\BOSS_Screenshots_"+ bossVersion+"\\SG_Sales Agent_"+error+".png"));
			pics[picCount++]="C:\\BOSS_Screenshots_"+ bossVersion+"\\SG_Sales Agent_"+error+".png";
		} catch (IOException e) {
	...
	public boolean manageRetail(){
		...
		try {
			assertTrue(isElementPresent(By.cssSelector("td.c.topb > #ec__c")));
		} catch (Error e) {
			return false;
		}
		new Select(driver.findElement(By.cssSelector("td.c.topb > #ec__c"))).selectByVisibleText("1%");
		driver.findElement(By.name("commit")).click();
		if(isElementPresent(By.cssSelector("div.toast-item.toast-type-error > p")))return false;
		try {
	...
	public void print(){
		...
		System.out.println("");
		System.out.println("**SG Sales Agent**");
		//login//////////////////////
		if(result[0]==1){
